    ...
    def __init__(self, layer_sizes=[512, 512], last_act=False):
        ...
        if last_act:
            for i, (in_size, out_size) in enumerate(
                ...
                self.MLP.add_module(
                    name="A{:d}".format(i), module=nn.LeakyReLU(
...
class NP_HEAD(nn.Module):
    def __init__(self, input_dim, latent_dim, num_classes=10,
                 memory_max_length=2560):
        super(NP_HEAD, self).__init__()

        self.latent_path = MLP(
            layer_sizes=[
                input_dim +
                num_classes,
                latent_dim,
    ...
    def forward(self, x_in, deterministic_memory, latent_memory, forward_times=10, phase_train=True,
                x_context_in=None, labels_in=None, labels_context_in=None, update_deterministic_memory=True):

        if phase_train:

            x_combine_deterministic_input = torch.cat(
                (x_context_in, labels_context_in), dim=-1)
            x_representation_deterministic = self.deterministic_path(
                x_combine_deterministic_input)
            ...
            x_target_in_expand = x_in.unsqueeze(
                0).expand(forward_times, -1, -1)
            context_representation_deterministic_expand = x_representation_deterministic.mean(
                0).unsqueeze(0).unsqueeze(1).expand(forward_times, B, -1)
            latent_z_target_expand = latent_z_target.unsqueeze(
                1).expand(-1, B, -1)

            if update_deterministic_memory:
                deterministic_memory = torch.cat(
                    (deterministic_memory, x_representation_deterministic.detach()), dim=0)
                if deterministic_memory.size(0) > self.memory_max_length:
                    Diff = deterministic_memory.size(
                        0) - self.memory_max_length
                    deterministic_memory = deterministic_memory[Diff:, :]

            latent_memory = torch.cat(
                (latent_memory, x_representation_latent.detach()), dim=0)
            ...
            decoder_input_cat = torch.cat(
                (latent_z_target_expand,
                 x_target_in_expand,
                 context_representation_deterministic_expand),
                dim=-1)
            ...
            context_representation_deterministic_expand = deterministic_memory.mean(
                0).unsqueeze(0).unsqueeze(1).expand(forward_times, B, -1)
